...
queries = { 
    ...
    "Race and gender combination with highest search rate": """
        ...
        (SUM(search_conducted) * 100.0 / COUNT(*)) AS search_rate
        FROM traffic_stops
    ...
    "Average stop duration for different violations": """
        SELECT violation, 
        AVG(stop_duration) AS avg_duration_minutes
        FROM traffic_stops
        GROUP BY violation
...
if st.button("Run Query"):
   ...
   # Plot using first column as X-axis and second column as Y-axis
...
if submit_button:
    new_data = (
        ...
        violation,
        violation,
        int(search_conducted),
        search_type if search_type else "",
        stop_outcome,
        int(is_arrested),
        stop_duration if stop_duration else "",
        int(drugs_related_stop),
        vehicle_number,
        stop_datetime_combined
    )
    ...
    summary = (
        ...
        f"and {'he' if driver_gender.lower() == 'male' else 'she'} received a {stop_outcome.lower()}. "
